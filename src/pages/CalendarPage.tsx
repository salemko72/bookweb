  ...
  return <section className="mx-auto max-w-[1600px] p-3 pb-8 md:p-5">
    ...
    <div className="mb-4 flex flex-col gap-3 lg:flex-row lg:items-end lg:justify-between">
      ...
      <div className="flex flex-wrap items-center gap-1.5">
        ...
        <label className="flex items-center gap-1.5 rounded-xl border border-slate-200 bg-white px-2 py-2 text-[10px] font-semibold text-slate-600">{t('sort')} <input aria-label={t('sortProperties')} type="checkbox" checked={sortOn} onChange={e=>setSortOn(e.target.checked)} className="accent-violet-600"/></label>
        ...
        <span data-testid="calendar-fit-label" className="w-12 text-[9px] font-semibold uppercase tracking-[0.1em] text-slate-500">{t('fit')} {fitMode ? t('on') : t('off')}</span>
        <button type="button" aria-label={t('previous')} onClick={() => moveRange(-14)} className="basis-full rounded-xl border border-slate-200 bg-white p-2 text-slate-600 hover:bg-slate-50 sm:basis-auto"><ChevronLeft size={16}/></button>
        ...
        <button type="button" aria-label={t('next')} onClick={() => moveRange(14)} className="rounded-xl border border-slate-200 bg-white p-2 text-slate-600 hover:bg-slate-50"><ChevronRight size={16}/></button>
      </div>
    </div>
