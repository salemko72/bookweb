...
export function CalendarPage({ role = 'viewer' }: { role?: UserRole } = {}) {
  ...
  return <section className="mx-auto max-w-[1600px] p-3 pb-8 md:p-5">
    ...
    <div className="mb-4 flex flex-col gap-3 lg:flex-row lg:items-end lg:justify-between">
      <div><p className="text-[10px] font-semibold uppercase tracking-[0.18em] text-violet-500">{t('availability')}</p><h1 className="mt-1 text-[4.59rem] font-light leading-[.95] tracking-tight sm:text-[5.78rem]">{focusedReservationId ? (reservations.find(r=>r.id===focusedReservationId)?.guest_name || t('booking')) : t('calendar')}</h1><p className="mt-1 text-xs text-slate-500">{focusedReservationId ? t('focusedBookingView') : t('continuousTimeline')}</p></div>
