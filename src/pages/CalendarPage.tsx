    ...
    <div ref={timelineRef} data-testid="calendar-timeline" className="overflow-x-auto rounded-[1.35rem] border border-[#ded7cd] bg-white">
      <div className="relative min-w-max" style={{ width: ROW_LABEL_WIDTH + days.length * dayWidth }}>
        ...
        {orderedProperties.map(property => {
          ...
          return <div key={property.id} data-property-id={property.id} data-testid="calendar-property-row" className={`relative z-10 flex border-b border-slate-100 transition-all ${draggedProperty===property.id?'opacity-65 scale-[0.995]':'opacity-100'} ${propertyDragTarget===property.id&&draggedProperty!==property.id?'bg-violet-50 ring-2 ring-inset ring-violet-300':''}`} style={{minHeight:Math.max(104,laneCount * LANE_HEIGHT + 15)}}>
            <div className="sticky left-0 z-[60] shrink-0 border-r border-slate-200 bg-white px-3 py-4" style={{width:ROW_LABEL_WIDTH}}><div className="flex h-full items-center gap-2 rounded-xl"><button type="button" disabled={sortOn} aria-label={t('sortProperties')} onPointerDown={e=>{if(sortOn)return;e.preventDefault();e.currentTarget.setPointerCapture(e.pointerId);setDraggedProperty(property.id);setPropertyDragTarget(property.id)}} onPointerMove={e=>{if(draggedProperty!==property.id)return;const row=document.elementFromPoint(e.clientX,e.clientY)?.closest<HTMLElement>('[data-property-id]');if(row?.dataset.propertyId)setPropertyDragTarget(row.dataset.propertyId)}} onPointerUp={e=>{if(e.currentTarget.hasPointerCapture(e.pointerId))e.currentTarget.releasePointerCapture(e.pointerId);if(propertyDragTarget)dropProperty(property.id,propertyDragTarget);setDraggedProperty(null);setPropertyDragTarget(null)}} onPointerCancel={()=>{setDraggedProperty(null);setPropertyDragTarget(null)}} className={sortOn?'hidden':'flex h-10 w-5 shrink-0 touch-none items-center justify-center rounded-lg text-slate-300 transition hover:bg-violet-100 hover:text-violet-600 active:cursor-grabbing'}><GripVertical size={17}/></button><div className="min-w-0 flex-1"><div className="flex items-start justify-between gap-2"><div className="h-14 w-14 shrink-0 overflow-hidden rounded-full bg-slate-100 ring-1 ring-slate-300 shadow-sm">{image ? <img src={image} alt="" className="h-full w-full object-cover"/> : null}</div><span className="mt-1 inline-flex h-3 w-12 shrink-0 rounded-full" style={{backgroundColor:property.color||'#7C5CFC'}} /></div><p className="mt-2 truncate text-base font-medium leading-none tracking-[-0.02em] text-slate-900" title={property.name}>{property.name}</p></div></div></div>
