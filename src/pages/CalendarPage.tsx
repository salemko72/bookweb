...
export function CalendarPage({ role = 'viewer' }: { role?: UserRole } = {}) {
  ...
  function dropProperty(target: string) { if (!draggedProperty || draggedProperty===target) return; const ids=orderedProperties.map(p=>p.id); const from=ids.indexOf(draggedProperty), to=ids.indexOf(target); ids.splice(from,1); ids.splice(to,0,draggedProperty); setPropertyOrder(ids); localStorage.setItem('calendar-property-order',JSON.stringify(ids)); setDraggedProperty(null) }
  ...
  return <section className="mx-auto max-w-[1600px] p-3 pb-8 md:p-5">
    ...
    <div className="mb-4 flex flex-col gap-3 lg:flex-row lg:items-end lg:justify-between">
      <div><p className="text-[10px] font-semibold uppercase tracking-[0.18em] text-violet-500">{t('availability')}</p><h1 className="mt-1 text-[2.7rem] font-light tracking-tight sm:text-[3.4rem]">{focusedReservationId ? (reservations.find(r=>r.id===focusedReservationId)?.guest_name || t('booking')) : t('calendar')}</h1><p className="mt-1 text-xs text-slate-500">{focusedReservationId ? t('focusedBookingView') : t('continuousTimeline')}</p></div>
    ...
    <div ref={timelineRef} data-testid="calendar-timeline" className="overflow-x-auto rounded-[1.35rem] border border-[#ded7cd] bg-white">
      <div className="relative min-w-max" style={{ width: ROW_LABEL_WIDTH + days.length * dayWidth }}>
        ...
        {orderedProperties.map(property => {
          ...
          return <div key={property.id} data-testid="calendar-property-row" draggable={!sortOn} onDragStart={()=>setDraggedProperty(property.id)} onDragOver={e=>e.preventDefault()} onDrop={()=>dropProperty(property.id)} className="relative z-10 flex border-b border-slate-100" style={{minHeight:laneCount * LANE_HEIGHT + 15}}>
            <div className="sticky left-0 z-[60] shrink-0 border-r border-slate-200 bg-white px-1.5 py-2.5" style={{width:ROW_LABEL_WIDTH}}><div className="flex items-center gap-1.5"><div className="h-7 w-7 shrink-0 overflow-hidden rounded-full bg-slate-100 ring-1 ring-slate-200">{image ? <img src={image} alt="" className="h-full w-full object-cover"/> : null}</div><div className="min-w-0"><p className="truncate text-[9px] font-semibold text-slate-800" title={property.name}>{property.name}</p><span className="mt-0.5 inline-flex h-1.5 w-6 rounded-full" style={{backgroundColor: property.color || '#7C5CFC'}} /></div></div></div>
            <div className="relative" style={{width:days.length * dayWidth}}>
              ...
              {layout.map(position => {
                ...
                return <div key={reservation.id} data-testid={`reservation-lane-${position.lane}`} className={`absolute flex items-stretch overflow-hidden rounded-xl ${blockConflict ? 'ring-2 ring-rose-600' : ''}`} style={{left, top:position.lane * LANE_HEIGHT + 4, width, height:BAR_HEIGHT}}>
                  <button type="button" disabled={!canEditReservation(role) || imported || savingReservationId === reservation.id} title={imported ? t('importedReadOnly') : t('resizeStart')} aria-label={t('resizeStart')} aria-busy={savingReservationId === reservation.id} onPointerDown={(e) => { e.stopPropagation(); if (!canEditReservation(role) || imported) return; setResizeMessage(null); setResizing({id:reservation.id,isBlock,propertyId:reservation.property_id,side:'start',initialX:e.clientX,initialCheckIn:reservation.check_in,initialCheckOut:reservation.check_out}) }} data-testid="reservation-resize-start" className={['z-20 flex w-4 shrink-0 items-center justify-center', imported ? 'cursor-default opacity-65' : 'cursor-ew-resize'].join(' ')} style={{backgroundColor:bg}}><EllipsisVertical size={12} strokeWidth={2.3} color={text}/></button>
                  ...
                  <button type="button" disabled={!canEditReservation(role) || imported || savingReservationId === reservation.id} title={imported ? t('importedReadOnly') : t('resizeEnd')} aria-label={t('resizeEnd')} onPointerDown={(e) => { e.stopPropagation(); if (!canEditReservation(role) || imported) return; setResizeMessage(null); setResizing({id:reservation.id,isBlock,propertyId:reservation.property_id,side:'end',initialX:e.clientX,initialCheckIn:reservation.check_in,initialCheckOut:reservation.check_out}) }} data-testid="reservation-resize-end" className={['z-20 flex w-4 shrink-0 items-center justify-center', imported ? 'cursor-default opacity-65' : 'cursor-ew-resize'].join(' ')} style={{backgroundColor:bg}}><EllipsisVertical size={12} strokeWidth={2.3} color={text}/></button>
                </div>
              })}
            </div>
          </div>
