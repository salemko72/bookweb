...
export function AgencyPage() {
  ...
  return <section className="mx-auto max-w-3xl p-4 pb-8 md:p-6"><header className="mb-5"><p className="text-[10px] font-semibold uppercase tracking-[.18em] text-violet-500">{hr?'ADMINISTRACIJA':'ADMINISTRATION'}</p><h1 className="mt-1 text-[3.672rem] font-light leading-[.95] tracking-tight sm:text-[4.624rem]">{hr?'Agencija':'Agency'}</h1><p className="mt-1 text-xs text-slate-500">{hr?'Identitet i regionalne postavke trenutne agencije.':'Identity and regional defaults for the current agency.'}</p></header>
