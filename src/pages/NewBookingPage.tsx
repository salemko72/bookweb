...
export function NewBookingPage() {
  ...
  return <section className="mx-auto max-w-3xl p-3 pb-8 md:p-5"><header className="mb-4"><button type="button" className="float-right mt-5 rounded-xl bg-slate-800 px-4 py-2 text-white" onClick={()=>setBlocking(true)}>{t('blockDates').toUpperCase()}</button><p className="text-[10px] font-semibold uppercase tracking-[0.18em] text-violet-500">{t('reservations')}</p><h1 className="mt-1 text-[2.938rem] font-light leading-[.95] sm:text-[3.7rem] tracking-tight">{t('newBooking')}<AgencyTitleMark /></h1><p className="mt-1 text-xs text-slate-500">{t('createBookingDescription')}</p><NightCountBadge start={form.checkIn} end={form.checkOut} className="mt-2" /></header>{blocking && <BlockDatesDialog properties={properties} propertyId={form.propertyId} start={form.checkIn} end={form.checkOut} onClose={()=>setBlocking(false)}/>}<form onSubmit={handleSubmit} className="space-y-3">
