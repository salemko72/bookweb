...
export function PropertiesPage({ role = 'viewer' }: { role?: UserRole } = {}) {
  ...
  function startCreate(){if(role!=='admin')return;setEditingId(null);setEditorOpen(true);setForm({...emptyForm});setPropertyReservations([]);setCalendarLinks([]);setDraftLinks([]);setLinksOpen(true);setNewLinkUrl('');setMessage(null);setError(null)}
  ...
  async function removeProperty(){if(!editingId||role!=='admin'||!window.confirm(t('confirmDeleteProperty')))return;setSaving(true);try{await deleteProperty(editingId);closeEditor();await load()}catch{setError(t('propertyActionFailed'))}finally{setSaving(false)}}
  ...
  return <section className="mx-auto max-w-6xl p-3 pb-8 md:p-5">
    ...
    <div className={`${editorOpen ? 'flex flex-col gap-4 lg:grid lg:grid-cols-[minmax(0,1fr)_430px]' : 'block'} items-start`}>
      ...
      {editorOpen&&canEditProperty(role)&&<section className="order-first lg:order-none rounded-[1.35rem] border border-[#e5ded3] bg-white p-4"><div className="flex items-start justify-between gap-3"><div><p className="text-[10px] font-semibold uppercase tracking-[0.16em] text-violet-500">{editingId?t('editProperty'):t('newProperty')}</p><h2 className="mt-1 text-lg font-semibold tracking-tight">{editingId?form.name||'Property':t('propertyDetails')}</h2></div><button type="button" onClick={closeEditor} aria-label="Close property editor" className="rounded-lg p-1.5 text-slate-400 hover:bg-slate-100"><X size={16}/></button></div>
        ...
        {editingId&&role==='admin'&&<button type="button" disabled={saving} onClick={()=>void removeProperty()} className="mt-5 w-full rounded-xl border border-rose-200 px-4 py-3 text-sm font-semibold text-rose-700 disabled:opacity-50">{t('deleteProperty')}</button>}</section>}
    </div>
  </section>
}
