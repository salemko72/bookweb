...
export function EditReservationPage({ role = 'viewer' }: { role?: UserRole } = {}) {
  ...
  return <section className="mx-auto max-w-3xl p-3 pb-8 md:p-5">
    ...
    <section className="mt-3 rounded-[1.35rem] bg-white p-4">
      ...
      <div className="mt-3 grid gap-3 md:grid-cols-2">
        <label className="text-[11px] font-semibold text-slate-600">{t('guests')}<input type="number" min={1} max={property?.capacity} value={extras.adults+extras.children || ''} disabled={readOnly} onChange={e=>{const total=Math.min(property?.capacity??999,Math.max(0,Number(e.target.value)||0));setExtras(current=>({...current,adults:Math.max(0,total-current.children),children:Math.min(current.children,total)}))}} onBlur={()=>{if(!readOnly && extras.adults+extras.children<1)setExtras(current=>({...current,adults:1,children:0}))}} className="mt-1 w-full rounded-xl border border-slate-200 px-3 py-2.5 text-sm font-normal disabled:bg-slate-50 disabled:text-slate-400"/></label>
        <label className="text-[11px] font-semibold text-slate-600">{t('source')}<select value={source} onChange={(e) => setSource(e.target.value as 'direct'|'agency')} disabled={readOnly} className="mt-1 w-full rounded-xl border border-slate-200 bg-white px-3 py-2.5 text-sm font-normal disabled:bg-slate-50 disabled:text-slate-400"><option value="direct">{t('directManual')}</option><option value="agency">{t('agency')}</option></select></label>
      </div>
    </section>
    ...
    <StayExtras value={extras} onChange={setExtras} start={checkIn} end={checkOut} guestName={guestName} onGuestName={setGuestName} disabled={!canEditReservation(role)||!contactReady}/>
