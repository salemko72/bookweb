...
export function EditReservationPage({ role = 'viewer' }: { role?: UserRole } = {}) {
  ...
  return <section className="mx-auto max-w-3xl p-3 pb-8 md:p-5">
    ...
    <section className="mt-3 rounded-[1.35rem] bg-white p-4">
      ...
      <div className="mt-3 grid gap-3 md:grid-cols-2">
        <label className="text-[11px] font-semibold text-slate-600">{t('guests')}<input type="number" min={1} max={property?.capacity} value={extras.adults+extras.children} readOnly className="mt-1 w-full rounded-xl border border-slate-200 px-3 py-2.5 text-sm font-normal disabled:bg-slate-50 disabled:text-slate-400"/></label>
