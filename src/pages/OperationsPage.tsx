...
function reservationPhaseLabel(phase: ReservationPhase, language: string) {
 const labels = language === 'hr'
  ? { current: 'AKTIVNI', upcoming: 'NADOLAZEĆI', past: 'ZAVRŠENI', cancelled: 'OTKAZANI' }
  : { current: 'CURRENT', upcoming: 'UPCOMING', past: 'PAST', cancelled: 'CANCELLED' }
 return labels[phase]
}
...
export function OperationsPage({ mode, role }: { mode: 'reservations' | 'guests' | 'tasks' | 'blocks'; role: UserRole }) {
 ...
 return <section className="mx-auto max-w-6xl space-y-4 p-4"><h1 className="text-[2.7rem] font-light capitalize tracking-tight sm:text-[3.4rem]">{mode === 'blocks' ? t('blockDates') : mode === 'tasks' ? t('tasksPage') : mode === 'guests' ? t('guestsPage') : t('reservations')}</h1>
 <div className="flex flex-wrap gap-2.5">{role!=='cleaning' && <><Link className="rounded-xl border border-slate-300 bg-white px-3 py-2 text-xs font-semibold text-slate-700 transition hover:border-violet-400 hover:text-violet-700" to="/reservations">{t('reservations')}</Link><Link className="rounded-xl border border-slate-300 bg-white px-3 py-2 text-xs font-semibold text-slate-700 transition hover:border-violet-400 hover:text-violet-700" to="/guests">{t('guestsPage')}</Link><Link className="rounded-xl border border-slate-300 bg-white px-3 py-2 text-xs font-semibold text-slate-700 transition hover:border-violet-400 hover:text-violet-700" to="/blocks">{t('blockDates')}</Link></>}<Link className="rounded-xl border border-slate-300 bg-white px-3 py-2 text-xs font-semibold text-slate-700 transition hover:border-violet-400 hover:text-violet-700" to="/tasks">{t('tasksPage')}</Link></div>
