...
export function OperationsPage({ mode, role }: { mode: 'reservations' | 'guests' | 'tasks' | 'blocks'; role: UserRole }) {
 ...
 return <section className="mx-auto max-w-6xl space-y-4 p-4"><h1 className="text-[4.59rem] font-light leading-[.95] capitalize tracking-tight sm:text-[5.78rem]">{mode === 'blocks' ? t('blockDates') : mode === 'tasks' ? t('tasksPage') : mode === 'guests' ? t('guestsPage') : t('reservations')}</h1>
