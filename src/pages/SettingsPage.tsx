...
import { Bell, Building2, CalendarDays, Languages, Moon, Ruler, SlidersHorizontal, Sun, UserRound, UsersRound } from 'lucide-react'
...
export function SettingsPage() {
  ...
  return <section className="mx-auto max-w-4xl p-3 pb-8 md:p-5">
    <header className="mb-5"><p className="text-[10px] font-semibold uppercase tracking-[0.18em] text-violet-500">Preferences</p><h1 className="mt-1 text-3xl font-semibold sm:text-4xl tracking-tight">{t('settings')}</h1><p className="mt-1 text-xs text-slate-500">Appearance, regional formatting and administration.</p></header>
    <div className="space-y-3">
      ...
      <section className="rounded-[1.35rem] bg-white p-4"><div className="flex items-center gap-3"><UsersRound size={18} className="text-violet-500"/><div><h2 className="text-sm font-semibold">{t('administration')}</h2><p className="text-[11px] text-slate-500">Open a section to manage the workspace.</p></div></div><div className="mt-3 grid gap-2 md:grid-cols-2">{adminItems.map(({path,key,hint,icon:Icon}) => <button key={path} type="button" onClick={()=>navigate(path)} className="flex items-center gap-3 rounded-xl bg-slate-50 px-3.5 py-3 text-left transition hover:bg-violet-50"><span className="flex h-8 w-8 shrink-0 items-center justify-center rounded-lg bg-white text-violet-600"><Icon size={15}/></span><span><span className="block text-xs font-semibold text-slate-800">{t(key)}</span><span className="block text-[10px] text-slate-500">{t(hint)}</span></span></button>)}</div></section>
    </div>
  </section>
}
