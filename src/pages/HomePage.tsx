...
type HomeEvent = { id: string; kind: 'arrival' | 'departure' | 'cleaning'; time: string; propertyId: string; guestName?: string; guests?: number; nights?: number; status: string }
...
function buildEvents(summary: DailyOperationalSummary): HomeEvent[] {
  return [
    ...summary.checkIns.map((reservation) => ({ id: `arrival-${reservation.id}`, kind: 'arrival' as const, time: reservation.check_in, propertyId: reservation.property_id, guestName: reservation.guest_name, guests: reservation.guests, nights: nightsBetween(reservation.check_in, reservation.check_out), status: 'Arriving' })),
    ...summary.checkOuts.map((reservation) => ({ id: `departure-${reservation.id}`, kind: 'departure' as const, time: reservation.check_out, propertyId: reservation.property_id, guestName: reservation.guest_name, guests: reservation.guests, nights: nightsBetween(reservation.check_in, reservation.check_out), status: 'Departing' })),
...
function EventRow({ event, property }: { event: HomeEvent; property?: Property }) {
  ...
  return <article data-testid={`home-event-${event.id}`} className="flex items-center gap-3 border-b border-[#ebe5dc] py-3 last:border-b-0 sm:gap-4">
    <div className="w-12 shrink-0"><p className="text-sm font-semibold tracking-tight text-slate-900">{timeLabel(event.time)}</p><p className="mt-0.5 text-[10px] text-slate-500">{event.kind === 'cleaning' ? 'Cleaning' : event.kind === 'arrival' ? 'Arrival' : 'Departure'}</p></div>
    ...
    <div className="min-w-0 flex-1"><p className="truncate text-sm font-semibold text-slate-900">{property?.name ?? 'Property'}</p>{event.kind === 'cleaning' ? <p className="mt-0.5 text-xs text-slate-500">Turnover cleaning</p> : <><p className="truncate text-xs text-slate-600">{event.guestName}</p><p className="mt-1 flex items-center gap-2 text-[10px] text-slate-500"><span className="inline-flex items-center gap-1"><UsersRound size={12}/>{event.guests} guests</span><span>{event.nights} nights</span></p></>}</div>
    <span className={`hidden shrink-0 items-center gap-1.5 rounded-xl px-3 py-2 text-[10px] font-semibold sm:inline-flex ${badgeClass}`}>{icon}{event.status}</span>
    <ArrowRight size={16} className="shrink-0 text-slate-300" />
  </article>
}

function DayPanel({ label, date, summary, properties, testId }: { label: string; date: string; summary: DailyOperationalSummary; properties: Property[]; testId: string }) {
  ...
  return <section data-testid={testId} className="rounded-[1.6rem] border border-[#e5ded3] bg-white p-4 shadow-[0_8px_24px_rgba(76,62,45,0.035)] sm:p-5">
    ...
    {events.length ? <div>{events.map((event) => <EventRow key={event.id} event={event} property={propertyById.get(event.propertyId)} />)}</div> : <div className="flex min-h-36 items-center justify-center rounded-2xl bg-[#fbfaf7] px-5 text-center"><div><CalendarDays size={20} className="mx-auto text-slate-300"/><p className="mt-2 text-sm font-semibold text-slate-700">No events</p><p className="mt-1 text-xs text-slate-500">Nothing scheduled for this day.</p></div></div>}
  </section>
}
...
export function HomePage({ onLogout }: HomePageProps) {
  ...
  return <section className="mx-auto max-w-6xl p-4 pb-8 md:p-7">
    <header className="mb-5 flex items-start justify-between gap-4"><div><h1 className="text-3xl font-light tracking-tight text-slate-900 md:text-5xl">Daily Overview</h1><p className="mt-2 text-base font-medium text-slate-700">{t('goodMorning')}</p><p className="mt-1 text-xs text-slate-500">{formatDate(new Date())}</p></div><button type="button" onClick={() => void handleLogout()} disabled={loggingOut} aria-label="Logout" title={t('logoutTitle')} className="flex shrink-0 flex-col items-center gap-1 text-[9px] font-semibold text-slate-500 disabled:opacity-50"><span className="flex h-11 w-11 items-center justify-center rounded-full border border-slate-300 bg-white shadow-sm hover:bg-slate-50"><LogOut size={18} strokeWidth={1.8}/></span><span>{loggingOut ? '…' : t('logout')}</span></button></header>

    <div data-testid="home-summary-grid" className="grid grid-cols-3 gap-2.5 sm:gap-3">{cards.map(([label, count, Icon, testName, iconClass]) => <div key={label} data-testid="home-summary-card" className="aspect-square rounded-[1.45rem] border border-[#e5ded3] bg-white p-2.5 shadow-[0_8px_24px_rgba(76,62,45,0.04)] sm:p-3.5"><div className="flex h-full flex-col justify-between"><span data-testid={`home-summary-icon-${testName}`} className={`flex h-10 w-10 items-center justify-center rounded-xl ${iconClass} sm:h-11 sm:w-11`}><Icon size={19} strokeWidth={1.9}/></span><div><p className="text-2xl font-semibold tracking-tight text-slate-900 sm:text-3xl">{count}</p><p className="mt-0.5 text-[9px] font-medium text-slate-500 sm:text-[10px]">{label}</p></div></div></div>)}</div>

    <div data-testid="home-day-grid" className="mt-5 grid gap-4 lg:grid-cols-2 lg:gap-5"><DayPanel label="Today" date={dateString(new Date())} summary={summary} properties={properties} testId="home-today-panel"/><div className="lg:border-l lg:border-[#e5ded3] lg:pl-5"><DayPanel label="Tomorrow" date={tomorrowDate} summary={tomorrowSummary} properties={properties} testId="home-tomorrow-panel"/></div></div>
  </section>
}
