...
export function HomePage({ onLogout }: HomePageProps) {
  ...
  return <section className="mx-auto max-w-6xl p-4 pb-8 md:p-7">
    <header className="mb-5 flex items-start justify-between gap-4"><div><h1 className="text-[3.1875rem] font-light leading-[.95] tracking-tight text-slate-900 md:text-[5.1rem]">{t('dailyOverview')}</h1><p className="mt-2 text-base font-medium text-slate-700">{t('goodMorning').replace('Kate', currentUser || 'there')}</p><p className="mt-1 text-xs text-slate-500">{formatDate(new Date())}</p></div><button type="button" onClick={() => void handleLogout()} disabled={loggingOut} aria-label="Logout" title={t('logoutTitle')} className="flex shrink-0 flex-col items-center gap-1 text-[9px] font-semibold text-slate-500 disabled:opacity-50"><span className="flex h-11 w-11 items-center justify-center rounded-full border border-slate-300 bg-white shadow-sm hover:bg-slate-50"><LogOut size={18} strokeWidth={1.8}/></span><span>{loggingOut ? '…' : t('logout')}</span></button></header>
