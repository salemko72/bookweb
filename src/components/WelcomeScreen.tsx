...
const copy = {
  hr: {
    ...
    known: 'Nedavno korištene agencije', back: 'Nazad', agencyTitle: 'Nova agencija', agencyBody: 'Ovo postaje odvojeni prostor za vaše nekretnine, rezervacije i tim.', agencyName: 'Naziv agencije', country: 'Država', language: 'Jezik', currency: 'Valuta', logo: 'Profilna slika agencije', optional: 'Neobavezno · JPG ili PNG do 2 MB', continue: 'Nastavi na Owner račun',
...
export function WelcomeScreen({ onAuthenticated }: { onAuthenticated: () => void }) {
  ...
  return <main className="min-h-[100svh] bg-[#f5f0e8] text-[#17243d]">
    ...
    <div className="mx-auto grid min-h-[100svh] w-full min-w-0 max-w-[1180px] items-stretch gap-0 overflow-hidden p-0 md:grid-cols-[1.08fr_.92fr] md:items-center md:gap-6 md:p-6">
      <section className="relative flex min-h-[38svh] min-w-0 overflow-hidden bg-gradient-to-br from-[#ddecf3] via-[#e5f1f5] to-[#cfe4ee] p-6 md:min-h-[calc(100svh-3rem)] md:rounded-[2.2rem] md:p-9"><div className="relative z-[1] min-w-0 max-w-lg"><p className="text-[10px] font-semibold uppercase tracking-[0.24em] text-sky-700/65">{c.eyebrow}</p><h1 className="mt-4 text-[2.25rem] font-light leading-[1.05] tracking-tight text-[#183a69] sm:text-5xl md:text-[3.35rem]">{c.title}</h1><p className="mt-4 max-w-md text-sm leading-6 text-slate-600">{c.body}</p><div className="mt-5 hidden gap-2 text-[11px] text-slate-600 sm:flex"><span className="rounded-full bg-white/55 px-3 py-2"><ShieldCheck className="mr-1 inline" size={14}/>{c.secure}</span><span className="rounded-full bg-white/55 px-3 py-2"><Sparkles className="mr-1 inline" size={14}/>{c.automatic}</span></div></div><div className="absolute -bottom-12 right-[-4rem] w-[75%] max-w-[520px] opacity-95 sm:-bottom-20 md:bottom-3 md:right-[-2rem] md:w-[78%]"><WelcomeIllustration/></div></section>
